...
from prototype.decode_layer import Decode
...
class BaseSSD:
    ...
    def detect(self, image, threshold=.5):
        """
        implementation in pure tf fashion
        """
        ...
        boxes, scores = Decode()([conf_pred[0], offsets_pred[0], anchors[0], threshold])
        return boxes.numpy(), scores.numpy()

    # def detect_(self, image, threshold=.5):
    #     """
    #     deprecated, detection implementation combined with tf model predictions and numpy operations
    #     """
    #     image = cv2.resize(image, (512, 512))
    #     input_image = image / 127.5 - 1.
    #     if input_image.ndim != 4:
